...
# Helper Functions
def fetch_items(size=None, condition=None, date=None):
    try:
        params = {k: v for k, v in {'size': size, 'condition': condition, 'posted_date': date}.items() if v and v != "All"}
        response = requests.get(f"{API_BASE}/items", params=params)
...
def display_item_card(item, col):
    with col:
        st.markdown(f'<div class="item-card"><div class="item-name">{item.get("name", "Unknown")}</div></div>', unsafe_allow_html=True)
        if st.button("View Details", key=f"item_{item.get('id', 0)}", use_container_width=True):
...
with tab1:
    ...
    with col1:
        ...
        size_filter = st.selectbox("Size", ["All", "XS", "S", "M", "L", "XL", "XXL"], label_visibility="collapsed")
    
    with col2:
        st.markdown('<div class="filter-label">Condition</div>', unsafe_allow_html=True)
        condition_filter = st.selectbox("Condition", ["All", "New", "Like New", "Good", "Fair", "Well-Worn"], label_visibility="collapsed")
    
    with col3:
        st.markdown('<div class="filter-label">Posted Date</div>', unsafe_allow_html=True)
        date_filter = st.selectbox("Posted Date", ["All", "Today", "Last 7 Days", "Last 30 Days", "Last 3 Months"], label_visibility="collapsed")
    
    st.markdown('</div>', unsafe_allow_html=True)
    
    items = fetch_items(size_filter, condition_filter, date_filter)
    ...
    if items:
        ...
        st.info("No items available matching your filters.")
